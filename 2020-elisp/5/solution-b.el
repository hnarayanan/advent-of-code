...
(defun list-difference (list1 list2)
  "Return a new list containing elements in LIST1 that are not in
LIST2."
  (seq-filter (lambda (item) (not (member item list2))) list1))

(defun identify-my-seat-id (all-seat-codes)
  "Go through all seat codes and find my seat ID"
  (let* ((all-taken-seat-ids (identify-all-seat-ids all-seat-codes))
         (min (apply #'min all-taken-seat-ids))
         (max (apply #'max all-taken-seat-ids)))
    (setq all-seat-ids (number-sequence min max))
    (car (list-difference all-seat-ids all-taken-seat-ids))))

(let ((all-seat-codes (read-seat-codes-data "input.txt")))
  (message "%d" (identify-my-seat-id all-seat-codes)))
