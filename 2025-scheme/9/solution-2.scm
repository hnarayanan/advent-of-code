...
;; The following procedures were generated by ChatGPT as an
;; optimisation. I spent days struggling with this and gave up.
;;
;; The original implementation was very slow because it checked every
;; point on a boundary. What this supposedly does is to check only the
;; points that cross boundaries in the perpendicular direction. This
;; dramatically reduces the number of checks and reduces the work by a
;; factor of N.
(define (pair-up xs)
  (if (null? xs)
      '()
...
(define (covers-interval? spans a b)
  (any (lambda (s) (and (<= (car s) a) (<= b (cdr s))))
       spans))

(define (filled-x-spans y h-boundaries v-boundaries)
  (let* (
         (xs (sort
              (filter-map
               (lambda (entry)
                 (let ((x (car entry))
                       (ranges (cdr entry)))
                   (and (any (lambda (r)
                               (in-range-open-high? y (car r) (cdr r)))
                             ranges)
                        x)))
               v-boundaries)
              <))
         (interior (pair-up xs))
         (h (assoc y h-boundaries))
         (hspans (if h (cdr h) '()))
         (all (append interior hspans)))
    (fold (lambda (r acc) (insert-range r acc)) '() all)))

(define (filled-y-spans x h-boundaries v-boundaries)
  (let* (
         (ys (sort
              (filter-map
               (lambda (entry)
                 (let ((y (car entry))
                       (ranges (cdr entry)))
                   (and (any (lambda (r)
                               (in-range-open-high? x (car r) (cdr r)))
                             ranges)
                        y)))
               h-boundaries)
              <))
         (interior (pair-up ys))
         (v (assoc x v-boundaries))
         (vspans (if v (cdr v) '()))
         (all (append interior vspans)))
    (fold (lambda (r acc) (insert-range r acc)) '() all)))
...
(define (edge-h-valid? y xmin xmax h-boundaries v-boundaries)
  (covers-interval? (filled-x-spans y h-boundaries v-boundaries) xmin xmax))

(define (edge-v-valid? x ymin ymax h-boundaries v-boundaries)
  (covers-interval? (filled-y-spans x h-boundaries v-boundaries) ymin ymax))
